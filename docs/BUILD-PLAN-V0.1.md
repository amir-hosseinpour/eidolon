...
- **NodeZero / Pentera** — commercial proprietary SaaS. Vendors won't open-source the governance layer because that's the moat. Eidolon ships it open-source; Voyageur sells the managed VMs and firm SOPs on top.
...
- **Voyageur (proprietary)** = Eidolon + White Tuque's managed VM provisioning (Proxmox SDN per-engagement VNETs, LUKS volume orchestration, Logger VM with chattr-immutable rsyslog) + firm-specific RoE templates, evidence handling SOPs, and the pre-loaded MSA scope language.
...
P-7. Update `PRD.md` to reflect the scope cut. Out-of-scope section gets the six VMs explicitly. ROADMAP.md gets a new top section "v0.1 = control plane only" and the six-VM bundle becomes Voyageur-internal.
...
- **Acceptance criteria.**
  ...
  - AC-4. `eidolon engagement erase <id>` writes a destruction event into the audit chain, returns a Cert of Destruction (signed JSON, plus a stub PDF in v0.1) referencing the LUKS volume UUID placeholder. (Real LUKS erase lives in Voyageur; v0.1 emits the *attestation*, the volume itself is BYO.)
...
- **Acceptance criteria.**
  ...
  - AC-5. The audit log file is opened with O_APPEND and the directory is mode 0700. (Real chattr +a is Voyageur-side on the Logger VM; v0.1 uses POSIX append + permissions.)
...
## DefCon Toronto May 2026 demo dress-rehearsal
...
5. **Demo machine fails at DefCon.** Mitigation: pre-record the 5-minute walk-through; if live demo crashes, play the recording.

## What this plan deliberately does NOT do

- Does not ship the six VMs (cut to Voyageur).
- Does not implement real LUKS erase (cut to Voyageur; v0.1 emits the attestation only).
- Does not implement chattr +a, TLS rsyslog, daily GPG rotation on a separate Logger VM (cut to Voyageur; v0.1 uses POSIX append on local disk).
