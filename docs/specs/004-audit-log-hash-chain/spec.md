...
- chattr +a / TLS rsyslog. Downstream forks ship those.
...
- AC-5. Audit directory is created with mode `0700`. Files are opened with `O_APPEND | O_CREAT | O_WRONLY`. (POSIX append; downstream forks Logger VM adds `chattr +a`.)
...
- Does the chain need to survive process restart with no replay? Yes for v0.1: chain reads from disk on each `append`, so process restart resumes correctly. Performance acceptable for v0.1 (single-digit ops/sec).
